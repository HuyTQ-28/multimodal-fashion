...
class RecDataset(object):
    ...
    def get_edge_timestamps(self):
        """
        Return an (E, 3) int64 array of [user_id, item_id, unix_timestamp]
        aligned to the current dataset split/order.
        """
        time_field = self.config['TIME_FIELD']
        ts_path = os.path.join(self.dataset_path, "edge_timestamps.npy")

        if os.path.exists(ts_path):
            edge_ts = np.load(ts_path, allow_pickle=False)
            ts_df = pd.DataFrame(edge_ts, columns=[self.uid_field, self.iid_field, time_field])
            ts_df = ts_df.drop_duplicates([self.uid_field, self.iid_field], keep="last")

            cur_edges = self.df[[self.uid_field, self.iid_field]].copy()
            cur_edges["_row_id"] = np.arange(len(cur_edges))

            merged = cur_edges.merge(ts_df, on=[self.uid_field, self.iid_field], how="left")
            if merged[time_field].isna().any():
                missing = int(merged[time_field].isna().sum())
                raise ValueError(f"edge_timestamps.npy is missing {missing} current split edges.")

            merged = merged.sort_values("_row_id")
            return merged[[self.uid_field, self.iid_field, time_field]].astype(np.int64).values

        if time_field not in self.df.columns:
            raise ValueError(
                f"Timestamp field {time_field} not loaded and edge_timestamps.npy not found."
            )

        return self.df[[self.uid_field, self.iid_field, time_field]].astype(np.int64).values
